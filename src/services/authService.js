...
const kakaoCallback = async (req, res) => {
  ...
  try {
    ...
    // 이미 존재하는 회원인지 검사
    con.query('SELECT * FROM user WHERE u_id = ?', [user.u_id], (err, results) => {
      if (err) {
        console.error('DB Error:', err);
        return res.status(500).send('회원 정보를 가져오지 못했습니다.');
      }
      ...
      if (results.length > 0) {
        ...
        con.query(
          'INSERT INTO user (u_id, u_name, u_nickname, u_email, u_birth, u_img) VALUES (?, ?, ?, ?, ?, ?)',
          [user.u_id, user.u_name, user.u_nickname, user.u_email, user.u_birth, user.u_img],
          (error, results) => {
            if (error) {
              console.error('DB Insert Error:', error);
              return res.status(500).send('회원 정보를 가져오지 못했습니다.');
...
module.exports = { naverCallback, kakaoCallback };
